...
class Monitoringplugin implements iApplicationUIExtension {
	...
	public function OnDisplayRelations(
		...
		// ) )) {
				try{
					$name=$oObject->Get ( 'friendlyname' );
				} catch (Exception $e){
					$name="Ticket";
				}
				$oPage->SetCurrentTab ( Dict::S ( 'QR code' ) );
				$sQRCodeURL = MetaModel::GetModuleSetting ( 'EnterpriseCmdbQRcode', 'qrcode_url', '' );
				$sHostURL = $sQRCodeURL . 'qrcode.php?name='.$name.'&class='.get_class ( $oObject ).'&id='.$oObject->Get ( 'id' );
				$oPage->add ( '	<img src="'.$sHostURL.'" alt="'.$sHostURL.'"></img>' );
		// }
	}
