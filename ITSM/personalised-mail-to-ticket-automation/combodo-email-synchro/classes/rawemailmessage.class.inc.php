 ...
 * This class is written in plain PHP using the standard functions base64_decode and quoted_printable_decode and depends only on the "iconv" extension for the functions iconv and iconv_mime_decode. The class can be used to process messages retrieved from POP3, IMAP or read from plain text files (.eml) Note that for dealing directly with a POP3 or IMAP server, the PHP extension 'IMAP' is probably much more efficient than this class!
 * This class is inspired from PlancakeEmailParser by Danyuki Software Limited (http://www.plancake.com) written Daniele Occhipinti (dan@plancake.com)
 *
...
class RawEmailMessage {
	...
	// Zorille added code
	public function SetRawContent(
			$rawContent) {
		$this->sRawContent = $rawContent;
		return $this;
	}
	...
	/**
	 * Retrieves the address(es) from the recipient of the message... "To". Note that in some cases there is no 'email', just a description like "undisclosed recipients"
	 *
	...
	/**
	 * Retrieves attachments of the message. Any part of the message that is not a "multipart", or an 'inline' text is considered as an attachment.
	 *
	...
	/**
	 * Saves the raw message to a file (basically creating the equivalent to a .eml file that can be read by Thunderbird, Lotus Notes or Outlok Express)
	 *
	...
	/**
	 * returns the first 'text/plain' part of the message that is not an attachment or null if no such part exists in the message
	 *
	...
	/**
	 * returns the first 'text/html' part of the message that is not an attachment or null if no such part exists in the message
	 *
	 ...
	 */
	public function StrictCharacterSetConversion(
			$bStrict = null) {
	...
	/**
	 * Extracts the headers and the raw body of the given part of the message The headers are returned as a hash array, with key (in lowercase) => decoded value The body is returned as an array of strings (one per line)
	 *
	 * @param array $aLines
	 *
	 ...
	 */
	static protected function DecodeHeaderString(
			$sInput) {
	...
	/**
	 * Finds the first part of the message that matches the given criteria: MIMEType / !Content-Disposition inside the (optional) part. If no part is given, scans the whole message.
	 *
	 ...
	 * @throws \Exception
	 */
	protected function FindFirstPart(
			$sMimeType,
			$sExcludeDispositionPattern = null,
			$aPart = null) {
		$aRetPart = null;
		if ($aPart === null) {
			$aPart = $this->aParts;
			...
			if (preg_match ( '/^([^;]+)/', $sContentType, $aMatches )) {
				...
				if (strcasecmp ( $sPartMimeType, $sMimeType ) == 0) {
					if ($sExcludeDispositionPattern != null) {
						...
						if (($sContentDisposition == '') || (preg_match ( $sExcludeDispositionPattern, $sContentDisposition ) == 0) || ($sPartMimeType === 'application/pkcs7-mime' && count ( $sSmimeType ) > 0 && $sSmimeType [1] === 'signed-data')) {
							$aRetPart = array ();
	...
	/**
	 * Decodes the 'lines' of the 'body' of the given part according to its headers (and the message's global headers) This function decode base64 and qencoded strings and converts the result to UTF-8 if needed
	 *
	 ...
	 * @return boolean
	 */
	protected static function IsNewLine(
			$sLine) {
	 ...
	 * @return boolean
	 */
	protected static function IsLineStartingWithPrintableChar(
			$sLine) {
		return preg_match ( '/^[A-Za-z]/', $sLine );
	}

	/**
	 * Tells whether a part of the message is an attachment or not based on its headers. The rules are: - multipart/<anything> is not an attachment - text/<anything> is an attachment if and only if the 'content-disposition' says so - all other types are considered as attachments
	 *
	...
	/**
	 * Turn the string of addresses into an array (one entry per address) where the two components of each address are separated: 'name' (human readable) and 'email'
	 *
	 ...
	 */
	protected static function ParseAddresses(
			$sAddresses) {
		$sTextQualifier = '"';
		$sAddressDelimiter = ',';
	...
	/**
	 * Tries to (!) separate the two components of each address 'name' (human readable) and 'email'
	 *
	 * @param string $sAddress The address to split
	 * @return array An array of ('name' => friendly_name, 'email' => email_address)
	 */
	protected static function ExtractAddressPieces(
			$sAddress) {
