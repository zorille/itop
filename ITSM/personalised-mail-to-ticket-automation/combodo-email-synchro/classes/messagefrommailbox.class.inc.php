...
class MessageFromMailbox extends RawEmailMessage {
	...
	public function Decode(
			$sPreferredDecodingOrder = 'text/plain,text/html') {
		...
		switch (strtolower($this->source)) {
			case 'oauth2' :
				$sBodyText = $this->aParts ['body'];
				$sBodyFormat = $this->aParts ['content-type'];
				break;
			default :
		...
		$aTos = $this->GetTo ();
		$aCCs = $this->GetCc ();
		$oMessage = new EmailMessage ( $this->sUIDL, $sMessageId, $sSubject, $sCallerEmail, $sCallerName, $sRecipient, $aReferences, $sThreadIndex, $sBodyText, $sBodyFormat, $aAttachments, $oRelatedObject, $this->GetHeaders (), $sDecodeStatus, $sDate, $aTos, $aCCs );
	...
	public function SendAsAttachment(
			$sTo,
			$sFrom,
			$sSubject,
			$sTextMessage) {
	...
	protected function ParseMessageId(
			$sMessageId) {
		$aMatches = array ();
		$ret = false;
