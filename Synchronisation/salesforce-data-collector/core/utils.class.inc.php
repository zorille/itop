...
class Utils
{
	...
	static public function CheckParameters($aOptionalParams)
	{
        ...
        if (is_array($argv))
        {
            ...
            foreach($argv as $iArg => $sArg)
            {
                ...
                {
                    if (
                        $aOptionalParams[$aMatches[1]] === 'text' &&
                        array_key_exists($aMatches[1], $aOptionalParams) &&
                        is_string($argv[$iArg + 1]) &&
                        substr($argv[$iArg + 1], 0, 2) !== '--'
                    ) {
                        $nextIsValue = true;
                        continue;
                    }
                    ...
                    elseif (
                        !array_key_exists($aMatches[1], $aOptionalParams) ||
                        ($aOptionalParams[$aMatches[1]] != 'boolean')
                    ) {
                        $aUnknownParams[] = $sArg;
                    }
                }
	...
	static public  function GetDataFilePath($sFileName)
	{
	    ...
	    $sPath = str_replace('%APPROOT%', substr(APPROOT, 0, -1), $sPath); // substitute the %APPROOT% placeholder with its actual value
